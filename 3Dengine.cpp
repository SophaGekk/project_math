...
*/
    ...
    readData(pp, cc, "resources/quad.vtk");
/*
...
*/
    ...
    pp.clear();
    cc.clear();
    readData(pp, cc, "resources/tri.vtk");
/*
std::cout << std::endl;
ITER(pp, i) std::cout << pp.at(i).x << '\t' << pp.at(i).y << '\t' << pp.at(i).z << std::endl;
ITER(cc, i) std::cout << cc.at(i).n[0] << '\t' << cc.at(i).n[1] << '\t' << cc.at(i).n[2] << '\t' << cc.at(i).n[3] << std::endl;
*/
    ...
    // for (int i = 0; i < 4; i++) {
    //     if (i < cc.size()) { // Check if i is within bounds
    //         for (int j = 0; j < 3; j++) {
    //             t.at(i).push_back({pp.at(cc.at(i).n[j]).x, pp.at(cc.at(i).n[j]).y, pp.at(cc.at(i).n[j]).z});
    //         }
    //     } else {
    //         // Handle the case when i is out of bounds (e.g., break the loop)
    //         break; // or continue;
    //     }
    // }
    for (int i = 0; i < 4; i++)
...
*/
    ...
    pp.clear();
    cc.clear();
    readData(pp, cc, "resources/penta.vtk");
